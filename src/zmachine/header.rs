...
impl ZHeader {
    pub fn new(memory: &Handle<ZMemory>) -> Result<ZHeader> {
        let z_version = ZVersion::new(memory.borrow().read_byte(ByteAddress::from_raw(OS_VERSION)))?;
    ...
    pub fn start_pc(&self) -> ByteAddress {
        let raw_value = self.memory.borrow().read_word(ByteAddress::from_raw(OS_START_PC));
        ByteAddress::from_raw(raw_value)
    }

    pub fn file_length(&self) -> usize {
        let raw_file_length = self.memory.borrow().read_word(ByteAddress::from_raw(OS_FILE_LEN));
